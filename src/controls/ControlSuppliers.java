...
public class ControlSuppliers implements ActionListener {
    ...
    public ControlSuppliers(ModelSuppliers modelSuppliers, ViewSuppliers viewSuppliers,Object controls[]){
        ...
        this.cas.vas.jbtn_back.addActionListener(this);
        this.ces.ves.jbtn_back.addActionListener(this);
    ...
    public void inView(){
        ...
        showData("Select * from proveedores");
    }
    ...
    public void back(){
        ...
        this.viewSuppliers.revalidate();
        this.viewSuppliers.repaint();
        this.viewSuppliers.setSize(857, 530);
        clean();
        showData("Select * from proveedores");
    }
    ...
    public void delete(){
        ...
        String supplier = ""+this.viewSuppliers.jt_suppliers.getValueAt(row, 1);
        if(JOptionPane.showConfirmDialog(viewSuppliers, "Deseas borrar al proveedor\n"+supplier,"Borrar", 2, 2) == 0){
            try {
                int value = (int) this.viewSuppliers.jt_suppliers.getValueAt(row, 0);
                cb.delete("delete from proveedores where id_proveedor ="+value);
            } catch (Exception e) {
    ...
    public void showData(String query){
        ...
        try {
            rs = cb.query(query);
            while(rs.next()){
                tblSuppliers.addRow(new Object[]{rs.getInt("id_proveedor"),rs.getString("nombre"), rs.getString("rfc"), rs.getString("calle"), rs.getString("colonia"), rs.getString("ciudad"),rs.getString("estado"), rs.getString("nombre_contacto"), rs.getString("telefono"), rs.getString("email")});
            }
        } catch (Exception e) {
            System.err.println(e.getMessage());
        }
    ...
    public void seach(String factor, String message){
        ...
        try {
            ...
            showData("Select * from proveedores where "+ factor +" = "+ "'"+field+"'");
        } catch (Exception e) {
    ...
    public void actionPerformed(ActionEvent ae) {
        if(ae.getSource() == this.viewSuppliers.jmi_add)
            add();
        else if(ae.getSource() == this.cas.vas.jbtn_back || ae.getSource() == this.ces.ves.jbtn_back)
            back();
